...
export default class AttackResolver {
  ...
  constructor(
    ...
    dieRoller: MultiDieRoller = new MultiDieRoller()
  ) {
  ...
  private getAttacksFromString(str: string): number[] {
    ...
    attackStrings.forEach(str => {
      attacks.push(parseInt(str));
    });
  ...
  resolveFullAttack(targetAc: number): FullAttackResult {
    ...
    attacks.forEach(attack => {
      const attackResult = this.resolveSingleAttack(
        targetAc,
        attack + 1,
        resolvedAttacks + 1,
        hits + 1
    ...
    if (isHit) {
      ...
      if (critThreat) {
        ...
        const modifiedConfirm =
          confirmRoll + bonusToHit + critConfirmBonus + attackBonus;
