<template>
  <div class="attack-settings-view">
    <div
      class="sim-settings-header"
      v-bind:style="{ background: attackSettings.color }"
    >
      {{ attackSettings.name }}
    </div>
    <div class="row">
      Attacks:<TooltipField
        fieldWidth="15em"
        v-model:value="attackSettings.attacks"
        :tooltip="attacksTooltip"
      />
    </div>
    <div class="row">
      Damage:<TooltipField
        v-model:value="attackSettings.damage"
        :tooltip="damageToolitp"
      />
    </div>
    <div class="row">
      Attack Bonus:<TooltipField
        fieldWidth="2em"
        v-model:value="attackSettings.attackBonus"
        :tooltip="attackBonusTooltip"
      />
    </div>
    <div class="row">
      Crit Threshold:<TooltipField
        fieldWidth="2em"
        v-model:value="attackSettings.critThreshold"
        :tooltip="critThresholdTooltip"
      />
    </div>
    <div class="row">
      Crit Confirm Bonus:<TooltipField
        fieldWidth="2em"
        v-model:value="attackSettings.critConfirmBonus"
        :tooltip="critConfirmTooltip"
      />
    </div>
    <div class="row">
      Crit Bonus Damage:<TooltipField
        v-model:value="attackSettings.critBonusDamage"
        :tooltip="critDamageBonusTooltip"
      />
    </div>
    <div class="row">
      DR:<TooltipField
        fieldWidth="2em"
        v-model:value="attackSettings.damageReduction"
        :tooltip="damageReductionTooltip"
      />
    </div>
    <div class="row">
      Mods:<TooltipField
        fieldWidth="15em"
        v-model:value="attackSettings.mods"
        :tooltip="modsTooltip"
      />
    </div>
  </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const critDamageBonusTooltip =
      "The bonus damage for critical hits.  This " +
      "is in addition to normal damage (i.e. <normal dmg> + <crit dmg>).  Format:\n1d8+2d6";
    const modsTooltip =
      "Conditional modifications to damage based on attack or" +
    ...
    const damageReductionTooltip =
      "Damage Reduction.  The amount to reduce each individual attack by.";
    return {
      attackSettings,
      attacksTooltip,
      damageToolitp,
