...
export default class HomeViewModel {
    ...
    printTimer(name: string, end = false) {
        ...
        const runningTime = ((endTime - (end ? this.startTime : this.lapTime)) / 1000).toFixed(3)
    ...
    runJob(ac: number, iteration: number, batchSize: number) {
        ...
        this.buttonTitle = 'Calculating (AC: ' + ac + ', iteration: ' + iteration + ')... Click to cancel'
        setTimeout(() => {
            for (let i = 0; i < batchSize; i++) {
                for (let r = 0; r < this.resolvers.length; r++) {
                    this.resultSet.addResult(index, r, this.resolvers[r].resolveFullAttack(ac))
                }
    ...
    caclulateClicked() {
        ...
        this.resultSet.colors = this.attackSettings.map((setting) => { return setting.color })
        this.resultSet.reset()

        this.resolvers = this.attackSettings.map((setting) => { return new AttackResolver(setting) })

        this.buttonTitle = 'Starting run...'
        const batchSize = Math.max(Math.min(parseInt(this.simSettings.iterations) / 100, 1000), 1)
        // eslint-diable-next-line
