<template>
  <span class="tooltip-field">
    <input
      v-bind:style="{ width: fieldWidth ?? '10em' }"
      v-model="fieldValue"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function positionTooltip(tooltip: HTMLElement) {
      ...
      if (
        sourceRect.x + tooltipWidth + sourceRect.width >
        window.innerWidth - averageScrollBarWidth
      ) {
        tipX = -tooltipRect.width;
      }
      if (
        sourceRect.y + tooltipRect.height + sourceRect.height >
        window.innerHeight - averageScrollBarWidth
      ) {
